...
let load = '<div class="bb-load" style="text-align: center;"><button class="load-btn button-load">加载更多</button></div>'
  ...
  fetch(bbUrl).then(res => res.json()).then( resdata =>{
    if(resdata){
      ...
      if(bbLoad){
        ...
        bbDom.insertAdjacentHTML('afterend', allnums);
      }
    }
  })
}
...
async function updateHTMl(data){
  ...
  // ✅ 首次渲染后再插入按钮
  if(!document.querySelector('.bb-load')){
    bbDom.insertAdjacentHTML('afterend', load);

    // ⚠️ 关键：按钮是新插入的，需要重新绑定事件
    let btn = document.querySelector("button.button-load");
    if (btn) {
      btn.addEventListener("click", function () {
        btn.textContent= '加载中';
